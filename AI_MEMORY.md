...
#   rg -n "#ANALYTICS" AI_MEMORY.md → vizitatori live, istoric pe zile și atribuirea traficului
...
#NOTIFY  Emailul „Ofertă/Campanie vizualizată" (`offerViewNotification.ts`) are acum sursa în TITLU: `👁 Campanie vizualizată · Google Ads — /olx — …`. `classifyTraffic(pageUrl, referrer)` (exportat, testat) → `Google Ads` (gclid/wbraid/gbraid/`gad_source=1`/utm cpc) / `Google organic` / `Căutare organică` / `Social` / `AI` / `Referral` / `Direct`; + rând „Sursă trafic" în body cu campania (`gad_campaignid`). #NOTIFY #ATTRIBUTION
#NOTIFY  Verificare disponibilitate în configurator (`Step1Date.tsx` „Verifică") → event live `availability_checked` (`POST /api/analytics/live/event`, `meta.date/dateKey/available`) → apare în `/admin/live` + **email** cu data verificată (subiect: `📅 Verificare disponibilitate: 15 August 2026 …`). Cooldown 10 min per `sessionId::availability_checked::<data>` (deci 1 email / dată distinctă). `useLiveVisitor` sare peste `.verify-btn` la `element_clicked` ca să nu dubleze. #NOTIFY
...
#ADMIN  Pagină nouă `/admin/proiectie` (`src/client/features/admin/components/ForecastPage.tsx`): proiecție pe ani viitori (anul curent + 4 ani, plus orice an mai îndepărtat unde există deja evenimente confirmate/finalizate). Pentru fiecare an poți seta target de nr. evenimente și/sau sumă EUR (`AdminSettings.yearlyTargets[an]`, persistat prin `PUT /api/admin/settings` ca orice alt câmp din `settings/admin`); progresul se calculează din evenimentele `confirmat`+`finalizat` cu `eventDate` în acel an (același criteriu ca GoalCard/FinancialSummary), cu lead-urile/tentativele afișate separat ca pipeline. #RECENT #ADMIN
...
#ADMIN  Documente firmă:       `/admin/company-documents` arhivează în Firestore `companyDocuments` și Bunny `company-docs/` certificate ONRC/CIF, acorduri vecini, drept de profesare la domiciliu, comodat, închiriere mașină PF→PFA și alte acte; accesul este doar pentru supreme admin, iar tabul rămâne disponibil și în `/admin/settings`. #ADMIN
...
#RECENT  2026-09-27: Notificările generice de vizitator (inclusiv returning/AI/Ads) sunt dezactivate server-side în triggerEvent la cererea explicită a proprietarului; analytics continuă separat. ActivityInbox șterge automat câte 400 de intrări istorice `type=visitor` la încărcare/poll prin DELETE /api/admin/activity/visitors și le ascunde imediat. Telefonul folosește acum type=contact, lead-urile rămân active. Acest comportament înlocuiește cooldown-ul generic descris mai jos. #NOTIFY #ADMIN
#RECENT  2026-09-27: Mailer central (inclusiv contracte/predări prin adaptorul mailer.sendMail) salvează emailDeliveries fără body/parole: pending/accepted/failed/unknown, maximum 3 încercări doar pentru refuz explicit SMTP 4xx, fără retry pentru timeout ambiguu/acceptare parțială/AUTH. Istoricul este best-effort; nu este coadă durabilă, pending după restart necesită verificare. Activity emailSent devine true numai după acceptarea SMTP. #NOTIFY
#RECENT  2026-09-27: Dashboard are EmailFailureBanner roșu mare deasupra widgeturilor, inclusiv în loading/error: orice eșec SMTP/verificare produce admin_settings/email_alert persistent, cu fallback în memoria procesului dacă Firestore cade. Poll 15s + focus; eroarea API afișează avertisment separat. Succesele nu șterg alerta; confirmarea explicită folosește id și tranzacție ca să nu ascundă o eroare mai nouă. Stack/message/code/responseCode/command/response sunt mascate pentru secretele cunoscute înainte de persistență și pot fi copiate din UI. Endpointuri protejate requireSupremeAdmin. #NOTIFY #ADMIN
#RECENT  2026-09-27: Activitate site → Email are Verifică SMTP, Trimite email de test (numai către adminUser.email), istoric 30 trimiteri și limită 30s între diagnostice. Acceptat SMTP nu înseamnă livrat în Inbox; nu există integrare de bounce/delivery webhook. Modificările necesită deploy; parola SMTP de producție nu este schimbată de acest cod. #NOTIFY #ADMIN

#RECENT  2026-09-27: `triggerEvent` rezervă cooldown-ul vizitatorului sincron, înainte de geo/settings/SMTP: rafalele concurente de pagini de la același IP+sursă produc maximum o notificare/intrare inbox la 18h, inclusiv la eșec SMTP. Booking are cheie separată de vizitator; telefonul păstrează cheia separată. Limitarea rămâne in-memory per proces (restartul o resetează), nu filtrează boți cu IP-uri diferite. Teste pentru rafală de 50 cereri, retry SMTP, lead/telefon și expirare. #NOTIFY #PITFALL

#RECENT  2026-09-26: GET instagram-proposals/album/:slug sortează rezultatele local după proposedAt, eliminând necesitatea indexului compus albumSlug+proposedAt. POST account-invitations raportează distinct eșecul Firebase Auth / Firestore / SMTP și loghează etapa. Cauza erorilor 500 raportate în producție necesită încă logul serverului; CSS 404 poate indica asset vechi după deploy. Validare: 21 teste țintite; typecheck. #ADMIN #PITFALL

#RECENT  2026-09-25: `imageFailureRecovery.ts` reîncarcă pagina după 6 poze distincte eșuate (dedupe origin+path fără token); maximum o reîncărcare/path/sesiune tab, protejată prin sessionStorage înainte de reload. RetryImage raportează după retry; imaginile obișnuite sunt verificate la 8s după eroare, doar dacă sunt încă montate și eșuate pe aceeași pagină. Dacă sessionStorage nu poate fi scris, refresh-ul automat este omis pentru a evita bucle. #MEDIA #PITFALL

#RECENT  2026-09-25: „Drafturi de lucru” include și contractele Firestore `status=draft`, nu doar formularele locale. Numără ambele surse, identifică prin data evenimentului + nume/„Draft necunoscut”, iar „Continuă draftul” pentru cele din cont deschide editorul existent `/admin/contracts/:id/edit`, fără duplicare în storage. #ADMIN #CONTRACTS

#RECENT  2026-09-25: Emailurile `[IMAGE LOAD FAILED]` au cooldown global de 6h în `monitoring.routes.ts`, indiferent de poză/token/pagină/IP; toate erorile continuă în dashboard. Rezervarea sincronă precedă emailul, inclusiv pentru rafale concurente. Cooldown-ul este in-memory (se resetează la restart), ca celelalte limitări existente. Test de regresie: 30 poze concurente → un email, următorul permis după 6h. #NOTIFY

#RECENT  2026-09-25: Audit linkuri 404: `/orase` genera toate combinațiile serviciu/localitate, inclusiv servicii fără rută pentru localitățile `services: ["nunta"]`; toate cele 31 URL-uri SEO distincte din ultimele 1000 serverErrors erau publicate acolo (42 înregistrări). CitiesHubPage și ServiceHubPage folosesc acum `allowedServices`, iar `SERVICE_HUB_SLUGS` centralizează cele 4 hub-uri existente pentru linkuri și publicRoutes. Live, adresele necunoscute răspund deja 302 spre `/oferta/olx?notFound=...`; referrer lipsește pentru exemplele investigate, deci proveniența fiecărei vizite nu e demonstrată. Separat, `/blog` a fost eliminat în commit 073d0d6, dar încă apare în sitemap/llms.txt (necorectat în acest patch). Validări: 5 teste HubLinks, typecheck, ESLint țintit, build complet. #LOCATION #SSR #PITFALL

#RECENT  2026-09-25: Contracte: drafturile de formular sunt independente în localStorage (`contract-work-draft:<id>`, query `?draft=<id>`), listate în ContractListPage cu data ca reper, nume sau „Draft necunoscut”, ultima modificare și stare manuală De făcut/În lucru. Păstrează inclusiv datele parțiale, clauzele și legătura cu evenimentul; la creare pe server se șterge doar draftul respectiv. Vechiul `contract-create-draft` migrează la deschiderea listei sau direct a formularului (unde este reluat automat dacă nu există context de eveniment); data singulară veche se restaurează și în lista zilelor. Drafturile de lucru sunt locale browserului, distincte de contractele `status=draft` din Firestore. Validări: typecheck, ESLint țintit, 9 teste Vitest, build. #ADMIN #CONTRACTS

#RECENT  2026-09-25: EventCard oferă „Exclude Calendar” / „Include Calendar”, persistat ca `adminEvents.excludeFromCalendar` (opțional, implicit false). Excluderea afectează exclusiv disponibilitatea publică (`publicBookedDates.routes.ts` și chatbot); calendarul admin păstrează evenimentul vizibil și verificările interne de contract rămân neschimbate. Validat cu `npm run typecheck` și 85 teste Vitest pentru adminEvents/publicBookedDates. #ADMIN

#RECENT  2026-09-25: Contractele noi pot fi salvate ca `draft` fără date client, inclusiv PJ/email. Linkul public cere semnatarului PJ să completeze datele beneficiarului (tip, denumire, CIF, înregistrare, sediu, oraș/județ, telefon, bancă/IBAN) și datele reprezentantului/email înainte de semnare; aceste câmpuri sunt persistate și intră în PDF. #RECENT #CONTRACTS
#RECENT  2026-09-25: Procesarea albumelor este durabilă în Firestore (`albumPhotoProcessingQueue`): fișierele pending rămân până la confirmarea uploadului Bunny, pornirea serverului și un sweeper reiau joburile incomplete, iar lista Bunny reconciliază uploadurile reușite chiar dacă serverul a căzut înainte de confirmare; retry cu backoff, email numai după completare și ștergerea jobului după email. #RECENT #MEDIA #ADMIN
#RECENT  2026-09-25: `runAlbumProcessing` filtrează întâi pozele fără WebP, apoi folosește numărul lor ca denominator în log/progres (`1/12`, nu `1/1037`); rezumatul separă generate, deja existente și eșuate. #RECENT #MEDIA #ADMIN
#RECENT  2026-09-25: Notificările de vizualizare ofertă/campanie detectează redirectul și din parametrul `notFound` al `pageUrl`, inclusiv când endpointul nu primește `redirectedFrom`; subiectul începe cu `🔀 Redirect de pe un link inexistent`, iar body-ul arată adresa originală. #NOTIFY #CAMPAIGN

#RECENT  2026-09-25: `PhoneNumberReveal` trimite microconversia Google Ads secundară „Telefon afișat” la dezvăluirea numărului pe toate paginile; landing pages trimit separat „Verificare disponibilitate” după o dată validă. Ambele au valoare 0 RON, numărare „Una”, fără dată sau alte date din formular către Ads. #RECENT #CAMPAIGN #ATTRIBUTION
#RECENT  2026-09-25: Rutele de pagină necunoscute sunt redirecționate HTTP 302 din SSR către `/oferta/olx?notFound=...`; `entry-server.tsx` verifică aceleași rute publice/admin/Wedding Hub prin `matchRoutes`. Asset-urile și API-urile inexistente rămân 404. Fallback-ul client `NotFoundPage` rămâne pentru navigări interne; accesările redirecționate direct de server nu mai montează acest component și nu mai trimit emailul său 404. #SSR #NOTIFY

#RECENT  2026-09-24: `usePageTracking` salvează imediat fiecare landing page public în `siteVisits` (inclusiv `/oferta/olx`) și păstrează `isBounce=true` până la prima interacțiune; heartbeat-ul/beacon-ul îl schimbă în `false`, iar `/admin/analytics` afișează eticheta „bounce — fără interacțiune”. #ANALYTICS #CAMPAIGN
#RECENT  2026-09-24: `/admin/live` refăcut cu selector zilnic (Europe/Bucharest, inclusiv DST), taburi Google Ads/Organic/Instagram/Facebook/TikTok/AI-LLM/Referral/Direct și audiență separată Clienți-Albume (`/media`). `src/shared/liveVisits.ts` centralizează clasificarea (UTM înainte de referrer; blogul nu dovedește sursa), limitele de prezență și intervalele zilelor. Istoricul filtrează `startedAtMs` server-side, cu paginare după document (inclusiv pagini complet arhivate); sumarul indică dacă lista este parțială. Validat: typecheck, ESLint țintit, 58 teste Vitest, build complet și browser desktop/mobil cu istoricul din 23 septembrie. #RECENT #ADMIN #ANALYTICS
#RECENT  2026-09-24: În `CampaignLandingPage.tsx`, `/oferta/olx` păstrează sus galeria parallax Portfolio imediat după hero și mută jos, după copyright, componenta `AncaVisualsPromo` (a doua galerie). Celelalte campanii păstrează `AncaVisualsPromo` în poziția inițială. #RECENT #CAMPAIGN
#RECENT  2026-09-24: Clickurile WhatsApp/telefon din landing pages trimit conversia „Persoană de contact” doar prin Google tag. A fost eliminat uploadul server-side paralel către aceeași acțiune, care putea dubla semnalul folosit de Smart Bidding; trackingul OAIQ rămâne separat. #RECENT #CAMPAIGN #ATTRIBUTION #PITFALL
#RECENT  2026-09-24: Vizitatorii din `/admin/live` sunt etichetați `Vizitator #N` pe zi: `numberDailyVisitors()` din `src/shared/liveVisits.ts` ordonează după prima sesiune a zilei și reutilizează numărul între sesiuni cu același cookie `visitorId`; istoricul zilei furnizează maparea completă, iar sesiunile live fără persistență încă primesc un număr provizoriu. #RECENT #ADMIN #ANALYTICS
#RECENT  2026-09-24: Precizie live: heartbeat-ul nu mai resetează inactivitatea, transmite vizibilitatea tabului și publică sesiunea completă; timeout-ul folosește ultimul semnal pentru durată. Sesiunile se restaurează din Firestore înainte de ingestion după restart, metadatele de atribuire se trimit la fiecare eveniment, iar `hasContactIntent`/`hasConfirmedLead` și audiența client rămân după plafonarea feedului. Submit-ul generic este `form_submit_attempted`; `sendLiveEvent` marchează submit-urile semantice reușite cu `confirmed: true`. UI nu acumulează timp pe sesiuni istorice fără endedAt și nu confundă eșecul încărcării cu zero vizite. #RECENT #ANALYTICS #PITFALL

#RECENT  2026-09-24: Crearea albumului din EventCard folosește `.keep` cu conținut, include `photos_preview` și verifică fiecare director în Bunny înainte de salvarea slug-ului/succes; erorile de acces nu mai sunt tratate ca folder inexistent, iar 409 nu mai apare ca succes în UI. Validat cu typecheck și testele adminEvents; mailer-ul este mock-uit în aceste teste. #ADMIN #MEDIA #PITFALL
#RECENT  2026-09-17: SEO Radar (`SeoRadarPage.tsx`) folosește un catalog separat cu municipii și orașe din Ardeal + Vâlcea pentru sugestia „Adaugă oraș”; comunele/satele din `locationData.ts` nu mai apar în datalist, iar orașele deja aflate în plan sunt excluse din opțiuni (normalizare fără diacritice). Detectarea analizelor păstrează și orașele vechi din `CITIES`. #RECENT #LOCATION #ADMIN
#RECENT  2026-09-08: „Roata surprizelor" din `CampaignLandingPage.tsx` redesenată complet — secțiune light (`bg-[#f6f2ea]`), header serif „Învârte roata surprizelor", roată cu 3 felii pastel (`conic-gradient(from -60deg, sage/pink/taupe)` → FOTOCABINĂ GRATUITĂ sus / VIDEOBOOTH 360 dreapta-jos / MAI ÎNCEARCĂ stânga-jos), etichete SVG orizontale pe 2 rânduri + iconițe, pointer auriu, buton central dark „ÎNVÂRTE" (nu se rotește, `spinPromo`). `targetAngle` acum 240 (spin 1) / 0 (win). La câștig: doar mesaj, **fără CTA „Verifică disponibilitatea"**. Timer jos.
#RECENT  2026-09-08: `CampaignLandingPage.tsx` (`/oferta/olx`) refăcut: (1) UN SINGUR formular — verificarea de disponibilitate din secțiunea `#verifica-data` are doar tip eveniment + dată (3 selecturi: zi / lună în cuvinte `MONTHS_RO_CAP` / an; default = anul curent + LUNA URMĂTOARE, ziua 1); citește `/api/booked-dates`, la „Verifică" afișează disponibil/ocupat și abia apoi opțional un mini-form nume+telefon („Lasă-ne numărul") sau WhatsApp. Fostul „CONTACT FORM" de jos a devenit doar CTA (Verifică / WhatsApp / Sună / Instagram `instagram.com/ancavisuals`). (2) Secțiunea galerie „Mai mult decât imagini frumoase" mutată imediat după HERO. (3) „Verifică" declanșează `reportAvailabilityCheck` (`utils/liveEvent.ts` → event `availability_checked` + email cu data). Anchor `#oferta` păstrat pe CTA-ul final. #RECENT #CAMPAIGN #CRO
#RECENT  2026-09-08: Vizitatori live (`/admin/live`), Faza 1 = LIVE TEXT (voce = Faza 2). Client: `hooks/useLiveVisitor.ts` (montat în `App.tsx`) trimite evenimente semantice la `POST /api/analytics/live/event` (+ `/ping` la 10s, `/end` pe `pagehide` via `sendBeacon`): `session_started, page_view, scroll_depth, pricing_viewed, gallery_viewed, gallery_load_more (.pg-load-more-btn), lightbox_opened, section_dwell ([data-track-section]), contact_clicked, whatsapp_clicked, phone_revealed, form_started/submitted, visitor_idle, session_ended`. Fără PII/valori din câmpuri. Sesiunea = `av_sid`/`av_vid` din `utils/visitorSession.ts` (extras din `usePageTracking`). Atribuire Google Ads: `sessionAttribution.ts` are acum `gclid/gad_source/wbraid/gbraid` + `isGoogleAdsSession()`. Server: `services/liveVisitors.service.ts` = store in-memory `Map<sessionId>` + `EventEmitter` + sweeper (timeout 35s, idle 60s) + persist debounced în Firestore `live_sessions`; `routes/liveVisitors.routes.ts` (public `/live/*` + SSE admin `GET /api/admin/analytics/live/stream` cu `requireFirebaseAuth+requireSupremeAdmin`, consumat cu `fetch`+`getReader` ca `AlbumHealthPage`). **Fără auto-ștergere** — sesiunile din `live_sessions` rămân permanent (cronul de retenție a fost eliminat la cererea clientului). Panoul `/admin/live` are tab **Istoric** (`GET /api/admin/analytics/live/history?limit&archived`) cu același feed `formatEvent`; arhivare soft via `POST /api/admin/analytics/live/:sessionId/archive {archived}` (flag `archived` pe doc, filtrare în cod ca să meargă și pe docurile vechi). `BOT_UA` mutat în `utils/botUa.ts` (partajat). Fără gating pe consimțământ (ca `siteVisits`). Emailurile critice existente (`/api/analytics/contact-click`, `lead_created`) rămân — `/live/event` doar `logActivity`, nu trimite email. **Dev**: în afara producției (`NODE_ENV!=="production"`) sunt bypass-uite AMBELE filtre server-side (`isAdminRequest` + `isLocalIp`) în `isTrackableRequest` din `liveVisitors.routes.ts`, plus guard-ul client `av_admin` (`isDevHost()` în `useLiveVisitor.ts`) — altfel cookie-ul de admin al ownerului îți respinge propriile evenimente și panoul nu poate fi testat local. Comutarea de tab NU mai închide sesiunea (doar `pagehide`); timeout heartbeat 75s. `form_submitted`/`form_started` sunt clasificate (`classifyForm` în `useLiveVisitor.ts`, doar din metadata câmpurilor, fără valori): `meta.kind` = **contact** (`/contact`,`/oferta/*`, form cu tel → critical „🎯 Un client vrea să fie contactat") / **delivery** (câmpuri adresă/județ/oraș → high „📦 Adresă de livrare completată") / **subscribe** (doar email, puține câmpuri → high „📧 Abonare la album") / **other**. Emailul + `logActivity` + `formatEvent` din `/admin/live` folosesc `FORM_KIND_TITLE`; cooldown email per `sessionId::form_submitted::<kind>`. Abonarea la album din `MediaAlbumPage.handleSubscribe` NU e `<form>` → apel explicit `sendLiveEvent("form_submitted", {meta:{kind:"subscribe"}})`.
**Orice click** pe `a/button/[role=button]/input[submit|button]` → event `element_clicked` cu `meta.text` (label buton) / `meta.href` / `meta.tag` (dedupe 1.5s client-side); cele speciale rămân `whatsapp_clicked/phone_revealed/contact_clicked/gallery_load_more`. **Email** către admin (`sendEventEmail` în `liveVisitors.routes.ts`, cooldown 10 min per `sessionId::event`) pentru cele 4 critice — conține vizitator #, sursă, campanie/gclid, locație + **event body JSON**. `element_clicked` NU trimite email. NB posibil dublu-email cu `/api/analytics/contact-click` (footer) — de consolidat dacă deranjează. #RECENT #ANALYTICS #ADMIN
