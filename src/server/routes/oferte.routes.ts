...
router.post("/:slug/view", async (req: Request, res: Response) => {
  try {
    ...
    if (settings?.email.offerViewed ?? true) {
      const context = await sendOfferViewNotification({
        slug,
        viewCount: newCount,
        request: req,
      ...
      logActivity({
        ...
        metadata: {
          ...
          clientName: offer.clientName ?? "",
          ip: context.ip,
          city: context.ipInfo?.city ?? "",
          region: context.ipInfo?.region ?? "",
          country: context.ipInfo?.country ?? "",
          userAgent: context.userAgent,
          referrer: context.referrer,
          viewCount: String(newCount),
        },
        emailSent: true,
      }).catch(() => {});
    } else {
      logActivity({
        ...
        title: `👁 Ofertă vizualizată — ${offer.clientName ?? slug}`,
        description: `/${slug} · vizualizare #${newCount}`,
        metadata: { slug, clientName: offer.clientName ?? "", ip: clientIp(req), viewCount: String(newCount) },
        emailSent: false,
      }).catch(() => {});
    }
