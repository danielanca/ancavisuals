...
import { conversationMapSecond } from './conversationMaps';
import { ConversationState, accountInterface, messageAccount, streamType } from './ConversationTypes';
import { getDynamicDocumentOrCollection, saveDiscussionStateToDB, db } from './getters';
...
type ChatResponse= {
  ...
  export class ForensicHandler {
    ...
    async getHistoryChat(data:any) : Promise<ChatResponse>{
        ...
        const chatRef = db.collection('accounts').doc(data.clientId)
                          .collection('departments').doc(data.departmentId)
                          .collection('conversations').doc('current')
                          .collection('messages');
        ...
        try {
            ...
            if (snapshot.empty) {
                ...
                let chatHistory = snapshot.docs.map(doc => {
