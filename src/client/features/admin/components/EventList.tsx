...
const EventList: React.FC<EventListProps> = ({ events, targetEventId, onAddEvent, onEventUpdated, onEventDeleted }) => {
  ...
  const tabs: { key: Tab; label: string; count: number; color: string }[] = [
    { key: "leaduri", label: "Lead-uri", count: filteredLeads.length, color: "bg-amber-500/20 text-amber-400" },
    { key: "viitor", label: "Viitoare", count: filteredUpcoming.length, color: "bg-emerald-500/20 text-emerald-400" },
    { key: "trecut", label: "Trecute", count: filteredPast.length, color: "bg-neutral-600 text-neutral-300" },
    { key: "arhiva", label: "Arhivă", count: filteredArchived.length, color: "bg-red-500/20 text-red-400" },
  ];
  ...
  const isStandaloneUpcomingLayout = tab === "viitor";
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
        ...
        {!statesLoaded ? (
          ...
          (tab !== "leaduri" && tab !== "arhiva") && (
            <div className={`${isStandaloneUpcomingLayout ? "" : "mb-3"} flex justify-end`}>
              <button
