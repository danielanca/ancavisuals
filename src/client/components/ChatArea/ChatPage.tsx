...
export const ChatPage: React.FC = () => {
  ...
  const parseHistoryToChatMessages = (historyString:string, departmentId:string): ChatMessage[] | null => {
   ...
   useEffect(() => {
      const fetchConversationHistory = async () => {
        ...
        const requestHistory:ChatMessage = {departmentId: chatProps.departmentID_URL, sender:'user', message:'REQUEST_HISTORY'};
        sendMessage(requestHistory).then(response=> {
          if (response.response.status !== 'SUCCESS') {
            ...
            const parsedMessages = parseHistoryToChatMessages(response.response.feedbackMessage.response, departmentID_URL);
  ...
  const handleSendMessage = () => {
    ...
    const newMessageOut : ChatMessage = { departmentId:departmentID_URL, sender:'user', message: inputText };
    ...
    sendMessage(newMessageOut).then( (response) => {
        ...
        if(response.response.status === 'SUCCESS'){
            ...
            const robotResponse : ChatMessage = { sender: 'robot', departmentId:departmentID_URL, message: feedbackMessage.response};
